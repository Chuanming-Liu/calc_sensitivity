...
if makemodel
    ...
    vecdisc = [30 100]; % layers - stop at the mantle
    veczkm = [5 10];
    vecptop = [5 7.6]; % P velocity at the shallow boundary
    vecpbot = [ 7 8.4]; % P velocity at the deep boundary
    
    
    vecstop = vecptop/1.85;
    vecsbot = vecpbot/1.85;
    vecrho = [ 2.9 3.38 ]; % g/m^3
    vecQK = [50000.0 50000.0];
    vecQS = [300.0 300.0];
...
CC = cbrewer('div','RdYlBu',length(ANperiods));
else
%     CC = cptcmap('GMT_split','ncol',length(periods));
    CC = cbrewer('div','RdYlGn',length(ANperiods));
% CC = cptcmap('GMT_temperature','ncol',length(ANperiods));
end
