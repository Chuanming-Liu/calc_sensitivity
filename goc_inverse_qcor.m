...
% ddata = param.data;
load('/Users/naccardo/Dropbox/Unix_DB/mcmc/data/final_phasev_results.mat');
...
%                 plot(periods,obs.Rphv,'ob','linewidth',2)
            ...
            forward.tphv = qcor.tphv;
            forward.tper = qcor.tper;
            forward.sphv = qcor.sphv;
            forward.sper = qcor.sper;
            ...
            % First SV
            % find perturbations that are over the allowed limit
            maxpert = 200;
            if maxpert > 0
                pertind = find(abs(mest.SV) > maxpert);
                if ~isempty(pertind)
                    pertSV = mest.SV;
                    for ip = 1:length(pertind)
                        ind = pertind(ip);
                        if pertSV(ind) > 0
                            pertSV(ind) = maxpert;
                        else
                            pertSV(ind) = -1*maxpert;
                        end
                    end
                else
                    pertSV = mest.SV;
                ...
                % Next SH
                pertind = find(abs(mest.SH) > maxpert);
                if ~isempty(pertind)
                    pertSH = mest.SH;
                    for ip = 1:length(pertind)
                        ind = pertind(ip);
